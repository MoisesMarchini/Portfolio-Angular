...
@Component({
  ...
  styleUrls: ['./background-blob.component.scss']
})
export class BackgroundBlobComponent implements AfterViewInit {

  svgAnimationInterval = 5000;
  currentSvgPath = 0;
  svgFullscreenPath = 'M -100, 200, -100, -200 H 100 V 200';
  svgPaths = [
  ...
  blobColors = environment.blobColors

  get isFullScreenBlob(){
    return environment.currentActiveSection !== 0;
  }

  get currentActiveSection(){
    return environment.currentActiveSection;
  }

  get boxMargin(){
    return environment.boxMargin[environment.currentActiveSection];
  }
  ...
  currentPath() {
    return this.svgPaths[this.currentSvgPath];
  } 
